...
*/
    startVx,
    startVy, //speed of balls at start
    ...
    backgroundGradient,
    icons, //background gradient, icon sprites
    audio, //audio context
...
*/
  ...
  function buildMenuHtml() {
    function makeButton(level, enabled, record) {
      return (
        "<p><button " +
        (enabled ? 'data-type="' + level.type + '"' : "disabled") +
        (level.intro ? ' data-intro="' + level.intro + '"' : "") +
        ">" +
        level.label +
        (record ? "<br>Your record: " + record : "") +
        "</button></p>"
      );
    }
    ...
    function getUnlockText(level, stars, prev) {
      ...
      if (level) {
        ...
        if (!prev && stars < level.stars) {
          html.push(
            "Finish the previous level and collect " + level.stars + " stars"
          );
        } else if (!prev) {
    ...
    html.push(
      '<p><button data-type="fullscreen">Toggle Fullscreen</button></p>'
    );
...
*/
  ...
  function initCanvas(callback) {
    ...
    function getAction(e) {
      ...
      x =
        ...
        TOTAL_WIDTH;
      y =
        ((e.clientY - e.target.offsetTop) / e.target.clientHeight) *
        TOTAL_HEIGHT;
      if (y < PANEL_HEIGHT) {
        if (x < PANEL_HEIGHT) {
          return isRunning
    ...
    backgroundGradient.addColorStop(
      1 - GRID / (TOTAL_HEIGHT - PANEL_HEIGHT),
      BACKGROUND_COLORS[1]
    );
    backgroundGradient.addColorStop(
      1 - (GRID - 1) / (TOTAL_HEIGHT - PANEL_HEIGHT),
      BACKGROUND_COLORS[2]
    );
    ...
    loadImage(
      SVG ? "data:image/svg+xml;base64," + btoa(SVG) : "icons.svg",
      GRID,
      12 * GRID,
      function (canvas) {
        icons = canvas;
  ...
  function resize() {
    ...
    scale = Math.min(
      isFullscreen() ? 2 : 1,
      docEl.clientWidth / TOTAL_WIDTH,
      docEl.clientHeight / TOTAL_HEIGHT
    );
  ...
  function draw() {
    ...
    allItems.forEach(function (item) {
      ...
      if (item.n > 0) {
        ...
        ctx.arc(
          SMALL_R + item.x,
          PANEL_HEIGHT + SMALL_R + INNER_HEIGHT - item.y,
          item.r,
          0,
          2 * Math.PI
        );
    ...
    animations.forEach(function (animation) {
      ...
      switch (animation.type) {
        case "remove-item":
          ...
          ctx.arc(
            SMALL_R + animation.x,
            PANEL_HEIGHT + SMALL_R + INNER_HEIGHT - animation.y,
            animation.r * state,
            0,
            2 * Math.PI
          );
    ...
    allBalls.forEach(function (ball) {
      ...
      ctx.arc(
        SMALL_R + ball.x,
        PANEL_HEIGHT + SMALL_R + INNER_HEIGHT - ball.y,
        SMALL_R,
        0,
        2 * Math.PI
      );
  ...
  function drawNumberBalls(n, x) {
    ...
    drawText(
      n,
      SMALL_R + x,
      PANEL_HEIGHT + SMALL_R + INNER_HEIGHT,
      2 * SMALL_R,
      2 * SMALL_R
    );
  }
  ...
  function drawLost(texts, texts2) {
    ...
    if (texts2) {
      start =
        TOTAL_HEIGHT / 2 + PANEL_HEIGHT / 2 + 0.75 * GRID * texts.length + GRID;
  ...
  function drawHelp() {
    ...
    drawText(
      ...
      TOTAL_WIDTH / 2,
      (3 * TOTAL_HEIGHT) / 4 + GRID,
      TOTAL_WIDTH / 2,
      GRID / 2
    );
    drawText(
      "to aim, Enter to shoot)",
      TOTAL_WIDTH / 2,
      (3 * TOTAL_HEIGHT) / 4 + 1.5 * GRID,
      TOTAL_WIDTH / 2,
      GRID / 2
    );
    ctx.textAlign = "center";
...
*/
  ...
  function waitEject(ballX, callback, v) {
    ...
    function moveHandler(x, y) {
      ...
      if (v) {
        ...
        ctx.beginPath();
        ctx.moveTo(
          SMALL_R + ballX + v.vx * l,
          PANEL_HEIGHT + SMALL_R + INNER_HEIGHT - v.vy * l
        );
        ctx.lineTo(
          SMALL_R + ballX + v.vx * t,
          PANEL_HEIGHT + SMALL_R + INNER_HEIGHT - v.vy * t
        );
        ctx.stroke();
      }
    }
  ...
  function addNewItems() {
    ...
    function getArray(n) {
      ...
      if (Math.random() < p) {
        a.push(
          Math.ceil((Math.floor(Math.random() * 2) / 2 + 1.5) * ballCount)
        );
      }
      if (Math.random() < p) {
        a.push(2 * ballCount);
      ...
      if (a.length < n) {
        ...
        if (Math.random() < p) {
          ...
          if (Math.random() < 0.3) {
            a.push(
              [-3, -5, -6, -7, -8][
                Math.floor(Math.random() * (3 * type[1] - 1))
              ]
            );
          }
          if (Math.random() < 0.3) {
    ...
    function doChaos(c, h, a) {
      ...
      for (i = 0; i < c; i++) {
        ...
        for (tries = 0; tries < 50; tries++) {
          (r =
            LARGE_R *
            [0.75, 0.75, 1, 1, 1, 1.2][Math.floor(Math.random() * 6)]),
            (x = r + (TOTAL_WIDTH - 2 * r) * Math.random() - SMALL_R);
          y = INNER_HEIGHT - r - Math.random() * 2 * GRID + SMALL_R - h;
          good = true;
          for (j = 0; j < allItems.length; j++) {
            if (
              pythagoras(x - allItems[j].x, y - allItems[j].y) <
              (r + allItems[j].r) * (r + allItems[j].r)
            ) {
              good = false;
              break;
            }
...
*/
  function getNextCollision(x, y, vx, vy, noWall) {
    ...
    allItems.forEach(function (item, index) {
      ...
      if (noWall) {
        //again with item on other side
        t = getCollisionTime(
          x - item.x + (vx < 0 ? -TOTAL_WIDTH : TOTAL_WIDTH),
          y - item.y,
          vx,
          vy,
          item.r + SMALL_R
        ...
        default:
          ...
          if (item.n > 0) {
            ...
            collision = getCollisionSpeed(
              ball.x -
                item.x +
                (ball.noWall && item.x > INNER_WIDTH / 2 ? TOTAL_WIDTH : 0),
              ball.y - item.y,
              ball.vx,
              ball.vy
            );
            ...
            if (item.n === 0) {
              ...
              animations.push({
                x:
                  ...
                  r *
                    (ball.x +
                      (ball.noWall && item.x > INNER_WIDTH / 2
                        ? TOTAL_WIDTH
                        : 0)),
            ...
            switch (-item.n) {
              ...
              case 7: //bomb
                ...
                allItems.forEach(function (otherItem) {
                  if (
                    pythagoras(otherItem.x - item.x, otherItem.y - item.y) <=
                    2 * r * r
                  ) {
                    otherItem.n = 0;
                  }
